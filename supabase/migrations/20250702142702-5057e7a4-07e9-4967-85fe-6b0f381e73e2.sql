...
-- Política para que los jugadores puedan ver entrenadores de clases activas
CREATE POLICY "Players can view trainers from active classes" 
  ON public.trainers 
  FOR SELECT 
  USING (
    is_active = true AND
    EXISTS (
      ...
      WHERE class_slots.trainer_id = trainers.id 
      AND class_slots.is_active = true
    )
  );
